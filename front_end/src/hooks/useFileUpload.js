...
export const useFileUpload = () => {
  ...
  const uploadImage = async (file, options = {}) => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || error.message || 'Зураг ачаалахад алдаа гарлаа!';
  ...
  const uploadMultipleImages = async (files, options = {}) => {
    ...
    try {
      if (showToast) {
        toast.loading(`${files.length} зураг ачаалж байна...`, { id: 'upload' });
      }
      ...
      if (response.success) {
        ...
        if (showToast) {
          toast.success(`${files.length} зураг амжилттай ачаалагдлаа!`, { id: 'upload' });
        }
  ...
  const deleteFile = async (filename, options = {}) => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || error.message || 'Зураг устгахад алдаа гарлаа!';
